...
from imagenet_model.quant_resnet_my import *
from imagenet_model.quant_conv import QConv
# from imagenet_model.quant_resnet_my_234 import *
# from imagenet_model.quant_conv_234 import QConv
...
def loading_data(args):
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.eval_size, shuffle=False,
...
def load_pretrained_fp_model(model):
    ...
    trained_fp_model = ptcv_get_model('resnet50', pretrained=True)

    old_checkpoint = trained_fp_model.state_dict()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == 'resnet50_quant':
        model = load_pretrained_fp_model(model, fp_path=fp_path)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i in range(0, len(args.bit_list)):
            ...
            # print("act_bit", int(args.bit_list[i]), 'weight_bit', int(args.bit_list[i]), acc1[i])
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank  == 0):
...
def train(train_loader, model, criterion, optimizer_m, optimizer_q, scheduler_m, scheduler_q, epoch, args, writer):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # print("\n[GPU {}]".format(args.gpu), i, "current random bits_train:", bits_train)
