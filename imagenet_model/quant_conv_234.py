...
class QConv_Tra_Mulit(nn.Conv2d):
    ...
    def select(self, weight, act):
        if self.first_conv:
            ...
            return weight[:act.shape[1], :act.shape[1], :, :], act
        else:
            return weight[:act.shape[1], :act.shape[1], :, :], act
    ...
    def select(self, weight, act):
        if self.first_conv: # only select output channel
            ...
            if self.weight_bit == 2 or self.weight_bit == 4:
                ...
                raise Exception("only support bit 2, 3, 4")
...
class SwitchableBatchNorm2d_Tra_Mulit(nn.Module):
    ...
    def forward(self, x):
        ...
        # if x.shape[1] != len(self.bn[index].weight):
        #     import IPython
        #     IPython.embed()
...
'''
# old version of SwitchableBatchNorm2d
class SwitchableBatchNorm2d(nn.Module):
